        ...
        if train_step_cnt <= 10 or train_step_cnt % args.print_interval == 0:
            ...
            if not args.not_write_log:
                ...
                wandb.log({"training loss": loss.item(), "training trans loss": trans_loss, "training rot loss": rot_loss, 
                        "training trans err": trans_err, "training rot err": rot_err, "scale err": scale_err  }, step= train_step_cnt)
            ...
            print('step:{:07d}, loss:{:.4f}, trans_loss:{:.4f}, rot_loss:{:.4f}, trans_err:{:.4f}, rot_err:{:.4f}, scale_err:{:.4f},  lr:{:.10f}   time: total:{:.4f} ld:{:.4f} ife:{:.4f} bp:{:.4f}'.format(
                train_step_cnt, tot_loss,    trans_loss,        rot_loss,        trans_err,        rot_err,        scale_err,         lr,          timer.last('step'),load_time,infer_time, bp_time)  )
