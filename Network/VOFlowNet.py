...
class VOFlowRes(nn.Module):
    def __init__(self, intrinsic=True, down_scale=True, config=1, stereo=False, autoDistTarget=0., uncertainty=0, out_feature=False):
...
if __name__ == '__main__':
    ...
    voflownet = VOFlowRes(down_scale=False, config=1, out_feature=False) # 
    voflownet.cuda()
    # print (voflownet)
    ...
    batch = 64
    channel = 2
    x, y = np.ogrid[:imsize2, :imsize1]
    # print (x, y, (x+y))
    img = np.repeat((x + y)[np.newaxis,...], channel, 0) / float(imsize2 + imsize1)
    img = np.repeat(img[np.newaxis,...], batch, 0) 
