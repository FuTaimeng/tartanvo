...
from imu_dataset import KITTI_IMU, TartanAir_IMU, imu_collate, move_to
...
class IMUCorrector(nn.Module):
    def __init__(self, size_list= [6, 64, 128, 128, 128, 6]):
        ...
        self.imu = pp.module.IMUPreintegrator(reset=True, prop_cov=False, gravity=-9.81007)
    ...
    def final_forward(self, data, init_state):
        feature = torch.cat([data["acc"], data["gyro"]], dim = -1)
        B = 1
        F = feature.shape[0]
        ...
        return self.imu(init_state = init_state, dt = data['dt'].reshape(1, -1, 1), gyro = corrected_gyro,
            acc = corrected_acc), output
...
if __name__ == '__main__':
    ...
    all_dataset = TartanAir_IMU(posefile=args.dataname, imudir=args.dataroot, mode="all")
    train_dataset = TartanAir_IMU(posefile=args.dataname, imudir=args.dataroot, mode="train")
    test_dataset = TartanAir_IMU(posefile=args.dataname, imudir=args.dataroot, mode="test")
    ...
    for epoch_i in range(args.max_epoches):
        ...
        print("epoch %d: train loss: %f test loss: %f "%(epoch_i, train_loss, test_loss))

    data = all_dataset.get_all_data()
    data = move_to(data, args.device)
    init_state = {
    "pos": data['init_pos'], 
    "rot": data['init_rot'][:1,:],
    "vel": data['init_vel'],}
    state, output = network.final_forward(data, init_state)

    import numpy as np
    np.savetxt("train_results/test_run/imu_delta.txt", output.detach().cpu().numpy().reshape(-1, 6))
